...
fn emit_ts_dispatch(out_dir: &Path) -> Result<(), CodegenError> {
    ...
    //
    // `platformInvoke` forwards (commandId, args, capability, tabId?) to the
    // Rust-side `dispatch_plugin_command` Tauri command which validates the
    // capability against `MountRegistry`, checks per-command permissions, and
    // routes to the plugin sidecar (sidecar routing lands with task11/M2).
    // `tabId` is plumbed through for correlation logging; until task17 wires
    // stable workspace IDs, callers may omit it and the Rust dispatcher logs
    // `tab_id = "unknown"`.
    let body = "// @generated by plugin-codegen. DO NOT EDIT.
...
export async function platformInvoke<Args, Result>(
  ...
  return invoke<Result>(\"dispatch_plugin_command\", {
    commandId,
    args,
    capability,
    tabId,
...
";
    ...
    // PluginCapability is a brand on `string`. The runtime value is the opaque
    // `cap_v1.<mount_uuid>.<nonce_b64url>` envelope the host minted at
    // `mount_plugin` time; the brand prevents callers from synthesizing a
    // capability out of an arbitrary string. The Rust `MountRegistry` is the
    // authoritative source of truth — frontend cannot forge a capability that
    // the dispatcher will accept (the nonce is hashed server-side and
    // compared in constant time).
    let capability_body = "// @generated by plugin-codegen. DO NOT EDIT.

import { invoke } from \"@tauri-apps/api/core\";

export type PluginCapability = string & { readonly __brand: \"PluginCapability\" };
...
export async function mountPlugin(
  pluginId: string,
  tabId?: string,
): Promise<{ handle: PluginCapability; mountId: string }> {
  const resp = await invoke<MountResponse>(\"mount_plugin\", { pluginId, tabId });
  return { handle: resp.handle as PluginCapability, mountId: resp.mountId };
}

export async function unmountPlugin(
  handle: PluginCapability,
  tabId?: string,
): Promise<void> {
  await invoke<void>(\"unmount_plugin\", { handle, tabId });
}
";
