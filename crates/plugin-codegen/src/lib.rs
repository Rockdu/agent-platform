...
fn emit_ts_dispatch(out_dir: &Path) -> Result<(), CodegenError> {
    ...
    //
    // Round 5 (task4) wired the real dispatcher: `platformInvoke` forwards
    // (commandId, args, capability) to the Rust-side `dispatch_plugin_command`
    // Tauri command which validates the capability against `MountRegistry`,
    // checks per-command permissions, and (post-task11) routes to the
    // appropriate plugin sidecar. The capability is no longer discarded.
    let body = "// @generated by plugin-codegen. DO NOT EDIT.
...
";
    ...
    // nonce the host minted at `mount_plugin` time; the brand prevents callers
    // from synthesizing a capability out of an arbitrary string. The Rust
    // `MountRegistry` is the authoritative source of truth — frontend cannot
    // forge a capability that the dispatcher will accept.
...
export async function mountPlugin(
  ...
  const resp = await invoke<MountResponse>(\"mount_plugin\", { pluginId });
  return { handle: resp.handle as PluginCapability, mountId: resp.mountId };
}

export async function unmountPlugin(handle: PluginCapability): Promise<void> {
  await invoke<void>(\"unmount_plugin\", { handle });
}
";
