...
| 1 | No architectural change. Operational/environment blocker recorded: Claude Code Bash tool does not inherit shell PATH; node toolchain unreachable. Captured as BitLesson `BL-20260516-claude-bash-path`. Round 1 stalled before implementation; mainline objective unchanged for Round 2. | Forward progress required user input on node binary path. Recording the operational blocker here so future review knows lack of code is environmental, not a goal-drift problem. | None — AC content unchanged. |
| 1 review | Corrected Round 1 blocker AC reference from AC-6.1 to task1 shell scaffold / AC-3.1 partial. | AC-6.1 is Papers/Zotero access and is not affected by React/Tauri shell scaffolding; the fixed orchestrator slot is part of task1 and later supports AC-3.1. | Prevents false progress/blocker accounting against AC-6. |
| 2 | Drift-recovery round delivered 5 `analyze` spec deliverables (task2, task9, task12, task14, task24) via codex while task1 toolchain blocker is unresolved. No architectural deviation; these specs are plan-listed deliverables that constrain downstream `coding` tasks (task3 codegen, task10 stdio helper, task13 claude wiring, task15 PTY actor, task25 confirm-on-write queue). Toolchain blocker (no cargo/rustc/node/npm on disk) remains the gating issue for task1 implementation. | Round 0/1 stalled with zero plan deliverables; Round 2 produced 5 substantive specs (1341 + 422 + 509 + ~300 + 341 ≈ 2900+ lines of normative spec content) directly from the plan's analyze task table, advancing 5/40 plan tasks. Task1 remains blocked by Operations/environment issue (toolchain absent), not architecture. | None — AC content unchanged; only delivery cadence affected. 5 tasks moved to completed-pending-verification. |
...
| task2: Write `docs/specs/plugin-contract.md` | AC-1.1, AC-1.4, AC-1.5 | completed (pending verification) | analyze | codex | M1 Phase B; deps task1. Round 2: 1341-line spec produced via codex; covers manifest schema, dispatcher contract, PluginCapability lifecycle, codegen pipeline, threat model, DOs/DON'Ts. |
...
| task9: Write `docs/specs/mcp-sidecar.md` | AC-2.1, AC-1.6 | completed (pending verification) | analyze | codex | M2 Phase A; deps task1. Round 2: 422-line spec; stdio framing rules, log discipline, sidecar identity scheme, lifecycle FSM, restart backoff, app-quit ordering. |
...
| task12: Write `docs/specs/claude-launch.md` | AC-2.3, AC-3.2 | completed (pending verification) | analyze | codex | M2 Phase D; deps task1. Round 2: 509-line spec; PATH discovery algorithm, per-tab MCP config generation, version handling, onboarding card UX, orchestrator launch flow. |
...
| task14: Write `docs/specs/terminal-events.md` | AC-8.2 | completed (pending verification) | analyze | codex | M3 Phase A; deps task1. Round 2: spec produced via codex covering TerminalEvent taxonomy, OSC agent marker, prompt-waiting heuristic, 1MB ring buffer policy, UTF-8/ANSI boundary scanner, dedup arbiter, notification payload shape. |
...
| task24: Write `docs/specs/confirm-on-write.md` | AC-7.1, AC-7.5 | completed (pending verification) | analyze | codex | M5 Phase A; deps task1. Round 2: 341-line spec; request protocol, operation ID idempotency, modal/queue behavior, terminal stdin pause-without-output-block, scope honesty wording for README + onboarding card. |
...
|-------|-----------------|-------------|-----------------|
| Node toolchain not in Claude Code Bash PATH (user installed node, but binary not in `/opt/homebrew/bin`, `/usr/local/bin`, `~/.nvm`, `~/.fnm`, `~/.volta`, `~/.asdf` — no usable `which node`) | 1 | AC-9.1; task1 shell scaffold (fixed orchestrator slot later supports AC-3.1) | User to run `which node` in interactive shell + share path; agent adds parent dir to `.claude/settings.local.json` `env.PATH`; agent verifies `node --version` works in Bash tool; then proceeds with task1 implementation in Round 2. |
